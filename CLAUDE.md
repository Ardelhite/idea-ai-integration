...
renders its streamed events as a multi-tab chat tool window. Three layers, one data flow:

```
ui/ (Swing, EDT) ──Listener──> core/FuguSession ──FuguTransport──> cli/ ──GeneralCommandLine──> codex
                                      ▲
                        owned/persisted by FuguSessionManager
```
...
- **`core/FuguSessionManager`** (`@Service(PROJECT)`, `PersistentStateComponent`) is the persistence
  hub. It owns the ordered list of chat tabs (one `FuguSession` each) and serializes them all into the
  **workspace file** (`StoragePathMacros.WORKSPACE_FILE`) so every tab survives restarts. The tool
  window mirrors this list as content tabs; the panel's "+" calls `create()` and closing a tab calls
  `remove()`. Tab titles are always renumbered to a clean `1..N` sequence on load.

- **`core/FuguSession`** is one tab: holds that tab's transcript (`messages`), dispatches turns, owns
  the approval/prompt callbacks, and captures/restores its own `FuguSessionState` (transcript + Codex
  `threadId`) for the manager. It is a **plain class, not a service** — never call `getService` for it;
  get tabs from the manager. The next turn resumes the same Codex thread via the captured `threadId`,
  and the transport is created lazily on first send so persisting an untouched tab spawns no process.

- **Persistence mapping (`core/FuguPersistence`).** The runtime transcript is a block model
  (`core/ChatModel`: `ChatMessage` → ordered `TextBlock`/`ToolBlock`s, so tool cards render inline).
  IntelliJ's XML serializer can't handle that, so `FuguPersistence` maps it to/from flat
  `Persisted*` beans. `toRuntime` reconstructs a minimal tool `input` JSON so `ToolCall` accessors
  still resolve after reload — keep the two models in sync when you add a block field.

- **Agent-file injection (`core/AgentContext`).** When `loadAgentContext` is on, the first send of a
  tab prepends the project's Claude-ecosystem instruction/memory files (`CLAUDE.md`, `.claude/*.md`,
  `~/.claude/.../MEMORY.md`, etc.) so Codex honours them. Injected **once per Codex thread**
  (`agentContextSent`); a restored tab is treated as already-injected. Codex reads its own `AGENTS.md`
  natively, so this only fills the Claude-side gap. All file reads run off the EDT.
